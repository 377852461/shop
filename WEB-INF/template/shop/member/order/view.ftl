...
function takeOver(sn){
	if(confirm("请你确认收到货以后再点击“确定”按钮，确认收货。")){
		$.ajax({
             type: "GET",
             url: "takeOver.jhtml",
             data: {sn:sn},
				...
				<div class="top">
					...
					<span class="handle">
						...
						[#if order.orderStatus!="completed"&&order.orderStatus!="refunded"]
							...
							[/#if]																				
						[/#if]						
						<a href="list.jhtml" class="button" style=" position:absolute;left:980px;top:520px;" >${message("shop.member.back")}</a>
					</span>
					<div class="tips">
						[#if order.expired]
						...
						[#elseif order.orderStatus == "partialRefunds" || order.orderStatus == "refunded"]
