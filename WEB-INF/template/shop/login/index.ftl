...
<title>${message("shop.login.title")}[#if systemShowPowered] - Powered By kuanda[/#if]</title>
					...
					<form id="loginForm" action="${base}/login/submit.jhtml" method="post">
						<table>
							[#if cscinit==0]
								<tr>
									<th>
										${message("shop.login.cscid")}:
									</th>
									<td>
										<input type="text" id="username" name="username" class="text" onkeyup="cleanpassword()" maxlength="${setting.usernameMaxLength}" />
									</td>
								</tr>
							[#else]
								<tr>
