...
[#if productCategory??]
	[@seo type = "productList"]
		<title>[#if productCategory.seoTitle??]${productCategory.seoTitle}[#elseif seo.title??][@seo.title?interpret /][/#if][#if systemShowPowered] - Powered By kuanda[/#if]</title>
	...
	[/@seo]
[#else]
	<title>${message("shop.product.title")}[#if systemShowPowered] - Powered By kuanda[/#if]</title>
	<meta name="author" content="kuanda Team" />
	<meta name="copyright" content="kuanda" />
[/#if]
...
$().ready(function() {
	...
	var $product_jrgwc = $(".product_jrgwc input");
...
function cart(id){
			...
			$.ajax({
				...
				cache: false,
				success: function(data) {
					if(data.message.content == '该商品库存不足'){
						alert(data.message.content);
					}else{
						location.href="/shop/cart/select.jhtml";
					}
				}
			});
								...
								[#list row as product]
									<li[#if !row_has_next] class="last"[/#if]>
										<a href="${base}${product.path}">
											...
											<span calss="product_name" title="${product.name}">${abbreviate(product.name, 25,"...")}</span>
											<span class="price">
										...
										<div class="product_jdjs"><div class="product_jdjs_content">${product.shortIntroduction}</div></div>
										<div class="product_jrgwc">
										 	<input type="button" value="加入购物车" id="${product.id}" />
										</div>									
									</li>
								[/#list]
							[/#list]
