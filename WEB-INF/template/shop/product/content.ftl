...
[@seo type = "productContent"]
	<title>[#if product.seoTitle??]${product.seoTitle}[#elseif seo.title??][@seo.title?interpret /][/#if][#if systemShowPowered] - Powered By kuanda[/#if]</title>
...
$().ready(function() {
	...
	//获取推荐商品
	$.ajax({
		url: "${base}/product/recommendProducts.jhtml",
		type: "GET",
		data: {ids: historyProductIds},
		dataType: "json",
		traditional: true,
		...
		success: function(data) {
			$.each(data, function (index, product) {
				var thumbnail = product.thumbnail != null ? product.thumbnail : "${setting.defaultThumbnailProductImage}";
				$recommendProducts.append('<li><a href="${base}' + product.path + '"><img src="' + thumbnail + '" \/>' + product.name + '<span>' + currency(product.price, true) + '<\/span><\/li><\/a>');
...
<body>
	...
	<div class="container productContent">
		...
		<div class="span18 last">
			...
			<div class="name">[#if product.validGroupon?has_content]<span style="color:red;">[团购]</span>[/#if]${product.name}[#if product.isGift] [${message("shop.product.gifts")}][/#if]
						[#if product.shortIntroduction?has_content](${product.shortIntroduction})[/#if]
						<div>
							<script type="text/javascript">
								(function(){
								var p = {
			...
			<div class="sn">
				...
				[/#if]
				<a href="${base}/store.jhtml?factoryId=${product.factory.id}" target="_blank" style="float:right;margin-right:16px;" >进入店铺</a>	
			</div>
			<div class="info">
				<dl>
			...
			<div>
				联系方式:<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=${product.factory.qq}&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:${product.factory.qq}:51" alt="点击这里给我发消息" title="点击这里给我发消息"/></a>
				<a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid=${product.factory.ww}&siteid=cntaobao&status=1&charset=utf-8"><img border="0" src="http://amos.alicdn.com/online.aw?v=2&uid=${product.factory.ww}&site=cntaobao&s=1&charset=utf-8" alt="点这里给我发消息" /></a>
			</div>
			[#if !product.isGift]
				<div class="action">
					[#if product.specifications?has_content]
			...
			<div id="bar" class="bar">
				<ul>
					...
					[/#if]
					[#if product.productSource?has_content]
						<li id="productSourceTab">
							<a href="#productSource">${message("shop.product.productSource")}</a>
						</li>
					[/#if]
					...
					<li id="shopCar">
						[#if product.isOutOfStock]
							<input type="button" id="addProductNotify_1" class="addProductNotify" value="${message("shop.product.addProductNotify")}" />
						[#else]
							<input type="button" id="addCart_1" class="addCart" value="${message("shop.product.addCart")}" />
						[/#if]
			...
			[/#if]
			[#if product.productSource?has_content]
				<div id="productSource" name="productSource" class="introduction">
					<div class="title">
						<strong>${message("shop.product.productSource")}</strong>
					</div>
					<div>
				...
				<div id="review" name="review" class="review">
					...
					<div class="content clearfix">
						[#if product.scoreCount > 0]
							...
							<div class="handle" style="width:93px;height:40px;">
							...
							<div class="score">
								<strong>${product.corpScore?string("0.0")}</strong>
								<div>
									<div class="score${(product.corpScore * 2)?string("0")}"></div>
									<div>${message("Product.scoreCount")}: ${product.scoreCount}</div>
								</div>
							</div>
							<div class="graph">
								<span style="width: ${(product.corpScore * 20)?string("0.0")}%">
									<em>${product.corpScore?string("0.0")}</em>
								</span>
								<div>&nbsp;</div>
								<ul>
									<li>${message("shop.product.graph1")}</li>
							...
							<div class="score">
								<strong>${product.customerScore?string("0.0")}</strong>
								<div>
									<div class="score${(product.customerScore * 2)?string("0")}"></div>
									<div>${message("Product.scoreCount")}: ${product.scoreCount}</div>
								</div>
							</div>
							<div class="graph">
								<span style="width: ${(product.customerScore * 20)?string("0.0")}%">
									<em>${product.customerScore?string("0.0")}</em>
								</span>
								<div>&nbsp;</div>
								<ul>
									<li>${message("shop.product.graph1")}</li>
										...
										[#list reviews as review]
											<tr>
												<th>
													${review.content}
													<div class="star">
														<div style="float:left;width: 60px;">${message("Review.productScore")}: </div>
														<div class="scoreleft score${(review.score * 2)?string("0")}"></div>
													</div>
													<div class="star">
														<div style="float:left;width: 60px;">${message("Review.corpScore")}: </div>
														<div class="scoreleft score${(review.corpScore * 2)?string("0")}"></div>
													</div>
													<div class="star">
														<div style="float:left;width: 60px;">${message("Review.customerScore")}: </div>
														<div class="scoreleft score${(review.customerScore * 2)?string("0")}"></div>
													</div>
												</th>
												<td>
													[#if review.member??]
