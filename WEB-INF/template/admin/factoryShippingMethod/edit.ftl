...
<title>${message("admin.shippingMethod.edit")} - Powered By kuanda</title>
...
$().ready(function() {
	...
	// 地区选择
	$areaId.lSelect({
		url: "${base}/admin/common/parentArea.jhtml"
	});
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
				...
				<td>
					<select name="defaultDeliveryCorpId">
						[#list deliveryCorps as deliveryCorp]
							<option value="${deliveryCorp.id}" [#if shippingMethod??&&shippingMethod.defaultDeliveryCorp??&&deliveryCorp.id==shippingMethod.defaultDeliveryCorp.id] selected="selected"[/#if]>
								${deliveryCorp.name}
							</option>
						[/#list]
					</select>
			...
			<tr>
				...
				<td>
					<span class="fieldSet">
						<input type="hidden" id="areaId" name="areaId" value="${(shippingMethod.area.id)!}" treePath="${(shippingMethod.area.treePath)!}" />
					</span>
				</td>
			</tr>
			<tr>
