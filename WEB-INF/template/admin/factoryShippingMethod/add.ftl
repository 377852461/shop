	...
	$inputForm.validate({
		rules: {
			...
			areaId:{
					required: true,
					remote: {
						url: "${base}/admin/factoryShippingMethod/check_areaId.jhtml?factoryId=${admin.factory.id}",
						cache: false
					}
				},
			continueWeight:{
		...
		messages: {
			areaId: {
				remote: "${message("admin.area.exist")}"
			}
		}
	});
				...
				<td>
					<select name="defaultDeliveryCorpId">
						...
						[#list deliveryCorps as deliveryCorp]
							<option value="${deliveryCorp.id}" [#if deliveryCorp??&&deliveryCorp==deliveryCorp]selected="selected" [/#if]>
									${deliveryCorp.name}
							</option>
						[/#list]
					</select>
