...
<title>${message("admin.order.view")} - Powered By kuanda</title>
...
$().ready(function() {
	...
	// 退货
	//$("#returnsButtons").click(function() {
		//location.href = "editReturn.jhtml?id="+$(this).next().val();	
	//});
	
	//确认退货
     $("#comfimreturnsButtons").click(function() {
		location.href = "confirmReturns.jhtml?id="+$(this).next().val();	
	});
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "unpaid" || order.paymentStatus == "partialPayment"|| order.paymentMethod.name=="货到付款")]
		// 支付
			...
			$.dialog({
				...
				[@compress single_line = true]
					...
					<form id="paymentForm" action="payment.jhtml" method="post">
						...
						<input type="hidden" name="paymentMethod" value="${order.paymentMethodName}" \/>
						<table class="input">
							<tr>
								<th>
									${message("Order.sn")}:
	...
	[#if !order.expired && (order.shippingStatus == "unshipped" || order.shippingStatus == "partialShipment")]
		// 发货
								...
								[#list order.orderItems as orderItem]
									...
									[#else]
										[#list orderItem.refundss as refunds]
											[#if refunds.refundsStatus=="expired"]
												
											[#elseif refunds.status=="refunded"||refunds.status=="accept"]
...
function fn1(id){
	location.href = "refundsConfirm.jhtml?id="+id;
}
function fn2(id){
	layer.prompt({title: '拒绝退款原因？'}, function(name){	
		location.href = "refuseRefunds.jhtml?id="+id+"&refundsFaild="+name;
	});
}

//取消退货
function cancelReturns(id){
	layer.prompt({title: '拒绝退货原因？'}, function(name){	
		location.href = "cancelReturn.jhtml?id="+id+"&confirmReturn="+name;
	});
};

//拒绝退货
function refuserefunds(id) {
	layer.prompt({title: '拒绝退货原因？'}, function(name){	
		location.href = "unReturn.jhtml?id="+id+"&confirmReturn="+name;
	});
};
//同意退货
function agreeReturn(id){
	location.href = "editReturn.jhtml?id="+id;	
};
...
<body>
	...
	<form id="agreeRefundsForm" action="refundsConfirm.jhtml" method="get">
		<input type="hidden" name="id" value="${order.id}" />
	</form>
	<form id="refuseRefundsForm" action="refuseRefunds.jhtml" method="get">
		<input type="hidden" name="id" value="${order.id}" />
	</form>
	<form id="returnsForms" action="editReturn.jhtml" method="get">
	  <input type="hidden" name="orderid" value="${order.id}" />
	  
	</form>
	<form id="unreturnsForms" action="unReturn.jhtml" method="get">
	   <input type="hidden" name="orderid" value="${order.id}" />
	   <input type="hidden" name="unreturn" value="5" />
	   <input type="hidden" name="unReturns" value="12" />
	</form>
	
	<form id="confirmreturnsForms" action="confirmReturns.jhtml" method="get">
	  <input type="hidden" name="orderid" value="${order.id}" />
	  <input type="hidden" name="returnSucess" value="14" />
	 
	</form>
	<form id="cancelRefundForms" action="cancelReturn.jhtml" method="get">
		<input type="hidden" name="orderid" value="${order.id}" />
	  	<input type="hidden" id="confirmReturn" name="confirmReturn"/>
	</form>
	...
	<table class="input tabContent">
		...
		<tr>
			...
			<td>
				[#if factory!=null && !order.expired && order.orderStatus == "unconfirmed" ]
					<input type="button" id="confirmButton" class="button" value="${message("admin.order.confirm")}" />					
				[/#if]
				[#if (!order.expired && order.shippingStatus=="unshipped" && order.orderStatus != "unconfirmed" && isShipping == "true") ]					
					[#if (order.paymentMethod.name=="货到付款"  ||  order.paymentStatus == "paid" ) && order.orderStatus != "completed"&& order.orderStatus != "cancelled"]
						[#if order.refundMoney<order.effectivePrice]
							<input type="button" id="shippingButton" class="button" value="${message("admin.order.shipping")}" />
						[/#if]
					[/#if]
				[/#if]
			</td>
			...
			<td>								
				[#if order.paymentStatus == "unpaid"&&order.paymentMethod.name=="货到付款"&& !order.expired && order.orderStatus!="unconfirmed" &&  order.orderStatus != "cancelled"]
					<input type="button" id="paymentButton" class="button" value="${message("admin.order.payment")}"/>
				[/#if]
				[#if order.shippingStatus=="shipped"&&order.paymentStatus == "paid"&&order.orderStatus == "returnsExecute"&&order.paymentMethod.name=="货到付款"]
					<input type="button" id="cancelButton" class="button" value="${message("admin.order.cancel")}"/>
				[/#if]
			</td>
		</tr>
		[/#if]
	...
	<table class="input tabContent">
		...
		[#list paymentOrders as paymentOrder]
			<tr>
				<td>
					${paymentOrder.payment.sn}
				</td>
				<td>
					${message("Payment.Method." + paymentOrder.payment.method)}
				</td>
				<td>
					${(paymentOrder.payment.paymentMethod)!"-"}
				</td>
				<td>
					${currency(paymentOrder.payment.amount, true)}
				</td>
				<td>
					${message("Payment.Status." + paymentOrder.payment.status)}
				</td>
				<td>
					[#if paymentOrder.payment.paymentDate??]
						<span title="${paymentOrder.payment.paymentDate?string("yyyy-MM-dd HH:mm:ss")}">${paymentOrder.payment.paymentDate}</span>
					[#else]
						-
					[/#if]
				</td>
	...
	<table class="input tabContent">
		<tr class="title">
			<th>
				退款单${message("Refunds.sn")}
			</th>
			<th>
				商品名称
			</th>
			<th>
				${message("Refunds.refundsReason")}
			</th>
			<th>
				${message("Refunds.refundsExplain")}
			</th>
			<th>
				${message("Refunds.method")}
			</th>
		...
		[#list refundss as refunds]
			<tr>
				...
				<td>
					${refunds.orderItem.name}
				</td>
				<td>
					${refunds.refundsReason.cause}
				</td>
				<td>
					${refunds.refundsExplain}
				</td>
				<td>
				...
				<td>
					[#if refunds.refundsImage != null ]
						<a href="${refunds.refundsImage}" target="_blank" ><img src="${refunds.refundsImage}" height="60px" ></a>
					[#else]
						--
					[/#if]
				</td>
				...
				<td>
					[#if refunds.status == "wait" && refunds.returns==null]
						<input type="button" class="button" onclick="fn1(${refunds.id})" value="${message("admin.order.agreeRefunds")}" [#if admin.username == "admin"] disabled="disabled" [/#if]/>
						<input type="button" class="button" onclick="fn2(${refunds.id})" value="${message("admin.order.refuseRefunds")}"[#if admin.username == "admin"] disabled="disabled" [/#if] />
					[/#if]
				</td>
			</tr>
		[/#list]
	...
	<table class="input tabContent">
		<tr class="title">
			<th>
				退货单${message("Returns.sn")}
			</th>
			<th>
				商品名称
			</th>
			<th>
				${message("Returns.returnReason")}
			</th>
			<th>
				${message("Returns.shippingMethod")}
			</th>
		...
		[#list returnss as refunds]
			<tr>
				...
				<td>
					${refunds.returns.returnReason}
				</td>
				<td>
					${(refunds.returns.shippingMethod)!"-"}
				</td>
				<td>
					${(refunds.returns.deliveryCorp)!"-"}
				</td>
				<td>
					${(refunds.returns.trackingNo)!"-"}
				</td>
				<td>
					${refunds.returns.linkName}
				</td>
				<td>
					<span title="${refunds.returns.createDate?string("yyyy-MM-dd HH:mm:ss")}">${refunds.returns.createDate}</span>
				</td>
				<td>
					[#if refunds.returns.returnsImage != null ]
						<a href="${refunds.returns.returnsImage}" target="_blank" ><img src="${refunds.returns.returnsImage}" height="60px" ></a>
					[#else]
						--
					[/#if]
				</td>
				...
				<td>
					[#if refunds.status == "wait" && refunds.returns != null  ]
						<input type="button" id="returnsButtons" class="button" value="${message("admin.order.returnTitle")}" onclick="agreeReturn(${refunds.id})" [#if admin.username == "admin"] disabled="disabled" [/#if] />
						<input type="button" id="unrefundsButton" class="button" onclick="refuserefunds(${refunds.id})" value="${message("admin.order.unreturns")}" [#if admin.username == "admin"] disabled="disabled" [/#if] />
					[#elseif refunds.status == "agreenreturn"&&refunds.returns.trackingNo!=null]
						<input type="button" id="comfimreturnsButtons" class="button" value="${message("admin.order.comfimreturnsButtons")}" [#if admin.username == "admin"] disabled="disabled" [/#if]/>
						<input type="hidden" value="${refunds.id}">
						<input type="button" id="cancelButtons" class="button" onclick="cancelReturns(${refunds.id})" value="${message("admin.order.cancelButtons")}" [#if admin.username == "admin"] disabled="disabled" [/#if]/>
					[#elseif refunds.status == "agreenreturn"]
						等待会员确认
					[/#if]
