...
<title>${message("admin.consultation.list")} - Powered By kuanda</title>
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as consultation]
					<tr [#if consultation.createDate=sysdate]style="color:red;"[/#if] >
					  <td>
							<input type="checkbox" name="ids" value="${consultation.id}" />
						</td>
						<td>
							<a href="${base}${consultation.product.path}" title="${consultation.product.name}" target="_blank" [#if consultation.createDate=sysdate]style="color:red;"[/#if]>${abbreviate(consultation.product.name, 50, "...")}</a>
						</td>
						<td>
						...
						<td>
							[#if admin.factory?? ]<a href="reply.jhtml?id=${consultation.id}">[${message("admin.consultation.reply")}]</a>[/#if]
