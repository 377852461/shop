...
<title>${message("admin.admin.edit")} - Powered By kuanda</title>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					${admin.factory.mName}
					<input type="hidden" name="factoryId" value="${admin.factory.id}" >
				</td>
			</tr>
			[/#if]
			[#if admin.employee.id ??]
			<tr>
				<th>所属员工:</th>
				<td>
					${admin.employee.number}&nbsp;&nbsp;${admin.employee.name}
					<input type="hidden" name="employeeId" value="${admin.employee.id}" >
				</td>
			</tr>
			[/#if]	
			<tr class="roles">
				...
				<td>
					<span class="fieldSet">
						[#list roles as role]
							<label>
								<input type="radio" name="roleIds" value="${role.id}" [#if admin.roles?seq_contains(role)] checked="checked"[/#if]  />${role.name}
							</label>
						[/#list]
					</span>
				</td>
