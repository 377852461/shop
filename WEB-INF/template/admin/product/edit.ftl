...
<title>${message("admin.product.edit")} - Powered By kuanda</title>
...
<script type="text/javascript" src="${base}/resources/admin/editor/kindeditor-min.js"></script>
...
$().ready(function() {
	...
	var $browserButton_group = $("#browserButton_group");
	...
	var $grouponTag = $("#grouponTag");
	var $grouponId = $("#grouponId");
	var $groupon = $("#groupon");
	var $grouponShow = $("#grouponShow")
	...
	var $storeCategoryShow=$("#storeCategoryShow");
	var $factoryId=$("#factoryId");
	var $storeCategoryId=$("#storeCategoryId");
	var $storeCategoryIdBank=$("#storeCategoryIdBank");
	
	[@flash_message /]
	
	$browserButton.browser();
	$browserButton_group.browser();
	
	[#if !(admin.factory)??]
		[#if !(product.factory)??]
			$("#storeCategoryTr").hide();
		[/#if]
		$factoryId.change(function(){
			if (this.value==null||this.value=="") {
				$("#storeCategoryTr").hide();
			} else {
				$("#storeCategoryTr").show();
			}
			$.ajax({
				url: "store_category.jhtml",
				type: "GET",
				data: {id: this.value},
				dataType: "json",
				cache: false,
				success: function(data) {
					$storeCategoryId.empty();
					$storeCategoryId.append('<option value="">请选择...</option>');
					$.each(data, function(i, attribute){
						$storeCategoryId.append('<option value="'+attribute.id+'">'+attribute.name+'</option>');
					});
				}
			});
		});
	...
	//添加进团购
	$grouponTag.bind("click", function () {
		$.ajax({
    		type:"GET",
    		url : "checkGroupon.jhtml?id=${product.id}",
    		success : function(data){
    			if(data==true){
    				if($grouponTag.attr("checked")=="checked"){
						$grouponShow.show();
					}else{
						$grouponId.val("");
						$groupon.val("");
    				...
    				$.message("error","进行中的团购不允许修改");
    				$grouponTag.attr({checked:"checked"});
    			}
    		}
    	})
    ...
    $groupon.autocomplete("groupon_select.jhtml", {    	
		dataType: "json",
		max: 20,
		width: 600,
		minChars: 0,
		...
		formatItem: function(item) {
			return '<span title="' + item.title + '">' + item.title.substring(0, 50) + '<\/span>';
		}
	}).result(function(event, item) {
		$grouponId.val(item.id);
		$groupon.val(item.title);
	});
	...
	$inputForm.validate({
		...
		submitHandler: function(form) {
			/*var temp=$("input[name='storeCategoryIds']")
			if(temp.length=="undefined"||temp.length<=0){
				$.message("warn", "没有选择店铺分类");
			}else */
			
			if($grouponTag.attr("checked")=="checked"&&$grouponId.val()==""){
				$.message("warn", "没有选择团购");
			}else{
	...
	//选择店铺商品分类
	$storeCategoryId.change(function(){
		var getId=$("#storeCategory"+this.value).html();
		if(this.value=="-1"){
			return null;
		}
		if(getId==null||getId=="undefined"){
			$storeCategoryShow.append("<a id='storeCategory"+this.value+"' onclick='$(this).remove();$(\"#storeCategoryId"+this.value+"\").remove();' >"+$(this).find("option:selected").text()+"&nbsp;&nbsp;&nbsp;&nbsp;</a>");
			$storeCategoryIdBank.append("<input id='storeCategoryId"+this.value+"' type='hidden' name='storeCategoryIds' value='"+this.value+"' />");
		}else{
			$("#storeCategory"+this.value).remove();
			$("#storeCategoryId"+this.value).remove();
		}
		$storeCategoryId.get(0).selectedIndex=0;
	});
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data"  >
		...
		<ul id="tab" class="tab">
			...
			<li>
				<input type="button" value="${message("admin.product.shortIntroduction")}" />
			</li>
			<!--<li>
				<input type="button" value="${message("admin.product.productSource")}" />
			</li>-->
			<li>
		...
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					<span class="fieldSet">
						<input type="text" name="groupImage" class="text" value="${product.groupImage}" maxlength="200" title="${message("admin.product.imageTitle")}" />
						<input type="button" id="browserButton_group" class="button" value="${message("admin.browser.select")}" />
						[#if product.groupImage??]
							<a href="${product.groupImage}" target="_blank">${message("admin.common.view")}</a>
						[/#if]
					</span>
				</td>
			</tr>
						...
						<select id="factoryId" name="factory_id">
							...
							[#list factorys as factory]
								[#if factory.isEnabled]
									<option value="${factory.id}"[#if product.factory.id==factory.id] selected="selected" [/#if]>${factory.mName}</option>
								[/#if]
							[/#list]
						</select>
					</td>
			...
			[/#if]
			<!-- StoreCategory Begin -->
			<tr id="storeCategoryTr">
				<th>
					${message("admin.factory.storeCategory")}:
				</th>
				<td>
					<div id="storeCategoryIdBank">
						[#list storeCategorys as storeCategory]
							<input id="${'storeCategoryId'+storeCategory.id}" type="hidden" name="storeCategoryIds" value="${storeCategory.id}" />
						[/#list]
					</div>
					<label id="storeCategoryShow">
						[#list storeCategorys as storeCategory]
							<a id="${'storeCategory'+storeCategory.id}" onclick="$(this).remove();$('#storeCategoryId'+${storeCategory.id}).remove();" >${storeCategory.name}&nbsp;&nbsp;&nbsp;&nbsp;</a>
						[/#list]
					</label>
					<select id="storeCategoryId" name="storeCategoryName" >
						<option value="-1">${message("admin.common.choose")}</option>
						[#list storeCategoryTree as productCategory]
			...
			<tr>
				<th>
					${message("Product.rakeback")}:
				</th>
				<td>
					<input type="text" name="rakeBack" class="text" maxlength="3" value="${product.rakeBack}"/>
				</td>
			</tr>
			<tr>
				<th>
			...
			<tr>
				...
				<td>
					<label>
						<input id="grouponTag" type="checkbox" [#if product.groupon!=null] checked="checked"[/#if] />团购
					</label>
					<input id="grouponId" type="hidden" name="grouponId" value="${groupon.id}"/>
					<span id="grouponShow" [#if groupon==null]class="hidden"[/#if]>请设置参与团购：<input type="text" id="groupon" class="text" maxlength="200" value="${groupon.title}"/></span>
				</td>
			</tr>
			<tr>
				<th>
		...
		<table class="input tabContent">
			<tr>
				<td>
					<textarea  name="shortIntroduction" style="width:70%;height:200px;visibility:hidden;">${product.shortIntroduction}</textarea>
				</td>
			</tr>
			<tr>
				<td>
					...
					<input type="hidden" id="word_count_1" value="" >				
				</td>
			</tr>			
		</table>
		<!--<table class="input tabContent">
			<tr>
				<td>
					<textarea id="editor3"  name="productSource" class="editor" style="width: 100%;">${product.productSource}</textarea>
				</td>
			</tr>
		</table>-->
