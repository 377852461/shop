...
<title>${message("admin.product.list")} - Powered By kuanda</title>
	...
	<form id="listForm" action="list.jhtml" method="get">
					...
					<div class="menuWrap">
						...
						<div class="popupMenu">
							<ul id="exportOrImportOption">
								<li>
									<a href="javascript:;" id="exportButton" >${message("admin.print.productList")}</a>
								</li>
							</ul>
						</div>
					</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="isMarketable">${message("Product.isAudit")}</a>
				</th>
				<th>
			...
			[#list page.content as product]
				<tr>
					<td>
						<input type="checkbox" name="ids" value="${product.id}" [#if (product.orderItems)?? && product.orderItems?size > 0 ] disabled="disabled"[/#if]/>
					</td>
					<td>
						${product.sn}
					</td>
					...
					<td>
						[#if product.stock??]
							[#if product.allocatedStock == 0]
								<span[#if product.stock<6] class="red"[/#if]>${product.stock}</span>
							[#else]
								<span[#if product.stock<6] class="red"[/#if] title="${message("Product.allocatedStock")}: ${product.allocatedStock}">${product.stock}</span>
							[/#if]
						[/#if]
					</td>
					<td>
					...
					<td>
						<span class="${product.isAudit?string("true", "false")}Icon">&nbsp;</span>
					</td>
					<td>
					...
					<td>
						...
						[#if product.isAudit]
							...
							[/#if]
							[#else]
							[${message("admin.product.notAudit")}]
						[/#if]
					</td>
				</tr>
			[/#list]
