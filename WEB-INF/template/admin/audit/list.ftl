...
<script>
	function aa(el){
		var $this=$(el),p=$this.closest("td"),val=p.find(":hidden").val();
		if(el.checked){
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as product]
				<tr>
					<td>
						<input type="checkbox" name="ids" id="${product.goods.id}" value="${product.id}" onclick="aa(this)"/>
						<input type="hidden" value="${product.goods.id}" id="goods">
					</td>
					<td>
						${product.sn}
					</td>
